                ...
                if (args.length == 1) {
                    ...
                    if (target != null) {
                        if (!target.getGameMode().equals(GameMode.SURVIVAL)) {
                            ...
                            player.sendMessage(ChatColor.GRAY + "You set " + ChatColor.WHITE + target.getName() + ChatColor.GRAY + "'s gamemode to " + ChatColor.LIGHT_PURPLE + "Survival");
                            target.sendMessage(ChatColor.GRAY + "Your gamemode was set to " + ChatColor.LIGHT_PURPLE + "Survival ");
                        } else {
                            player.sendMessage(target.getName() + ChatColor.RED + " is already in gamemode " + ChatColor.LIGHT_PURPLE + "Survival");
                        }
                    } else {
