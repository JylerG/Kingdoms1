            ...
            case 2:
                ...
                if (args[0].equalsIgnoreCase("get")) {
                    if (player.hasPermission("kingdoms.get")) {
                        ...
                        /*
                        //TODO: Fix this to display the owner, admins, and members, and if they don't exist, send a message saying they don't exist
                        player.sendMessage(args[1] + ChatColor.RED + " does not exist");
                        String kingdomName = args[1];

                        player.sendMessage(ChatColor.LIGHT_PURPLE + kingdomName + ChatColor.DARK_PURPLE + " Owner: " + ChatColor.WHITE + plugin.getKingdomOwner().get(kingdomName));
                        player.sendMessage(ChatColor.LIGHT_PURPLE + kingdomName + ChatColor.DARK_PURPLE + " Admins: " + ChatColor.WHITE + plugin.getKingdomAdmin().get(kingdomName));
                        player.sendMessage(ChatColor.LIGHT_PURPLE + kingdomName + ChatColor.DARK_PURPLE + " Admins: " + ChatColor.WHITE + "None");
                        player.sendMessage(ChatColor.LIGHT_PURPLE + kingdomName + ChatColor.DARK_PURPLE + " Members: " + ChatColor.WHITE + plugin.getKingdomMember().get(kingdomName));
                         */
                ...
                if (args[0].equalsIgnoreCase("create")) {
                    if (!plugin.getKingdoms().containsKey(player.getUniqueId().toString())) {
                        if (!plugin.getKingdoms().containsValue(args[1])) {
                            ...
                            plugin.getKingdomOwner().put(args[1], player.getName());
                            plugin.getKingdomAdmin().put(args[1], player.getName());
                            plugin.getKingdomMember().put(args[1], player.getName());
                            plugin.saveMember();
                            plugin.saveAdmin();
                            plugin.saveOwner();
                ...
                if (args[0].equalsIgnoreCase("disband")) {
                    if (!player.hasPermission("kingdoms.disband.admin")) {
                                ...
                                if (plugin.getOwner().get(player.getUniqueId().toString()).equals(args[1])) {
                                    if (plugin.getKingdoms().containsValue(args[1])) {
                                        for (Player p : Bukkit.getOnlinePlayers()) {
                                            plugin.getKingdomOwner().remove(args[1], p.getName());
                                            plugin.getKingdomAdmin().remove(args[1], p.getName());
                                            plugin.getKingdomMember().remove(args[1], p.getName());
                        ...
                        if (plugin.getKingdoms().containsValue(args[1])) {
                            ...
                            Iterator<Map.Entry<String, String>> iterator = plugin.getClaimedChunks().entrySet().iterator();

                            while (iterator.hasNext()) {
                                Map.Entry<String, String> claims = iterator.next();

                                // Check if the value matches args[1]
                                if (claims.getValue().equals(args[1])) {
